...
void show_board_or_error(int valid, int input_col, int input_row, void (*print_board)(int, int)); // 盤面表示またはエラー表示関数のプロトタイプ
...
void print_board(int input_col, int input_row); // 盤面表示関数のプロトタイプ
...
int main() {
    ...
    while (!is_game_end(tries, found)) { // ゲーム終了条件を満たすまで繰り返す
        ...
        show_board_or_error(valid, input_col, input_row, print_board); // 盤面またはエラー表示
...
// print_boardをmain関数内のローカル関数として利用するため、関数ポインタを使う
void show_board_or_error(int valid, int input_col, int input_row, void (*print_board)(int, int)) {
    if (valid) { // 妥当な入力なら
...
int is_game_end(int tries, int found) {
    return ((tries >= MAX_TRIES) || (found != 0)); // 最大回数または発見で終了
}
